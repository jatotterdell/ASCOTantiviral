...
names(res$drws$OR)[-1] <- c("Ineligible aspirin", "Age \u2265 60", "Oxygen requirement", "Australia/New Zealand", "Nepal")
```
...
```{r}
res <- fit_primary_model(fas_itt_nona_dat, logistic_site_epoch, ctr = contr.treatment)
names(res$drws$AOR) <- "Nafamostat"
names(res$drws$COR) <- intervention_labels2()$CAssignment[-(1:2)]
names(res$drws$OR)[-1] <- c("Ineligible aspirin", "Age \u2265 60", "Oxygen requirement", "Australia/New Zealand", "Nepal")
```
...
res <- fit_primary_model(
  ...
  beta_sd_int = 10, beta_sd_trt = 10, beta_sd_var = rep(10, 5)
)
names(res$drws$AOR) <- "Nafamostat"
names(res$drws$COR) <- intervention_labels2()$CAssignment[-(1:2)]
names(res$drws$OR)[-1] <- c("Ineligible aspirin", "Age \u2265 60", "Oxygen requirement", "Australia/New Zealand", "Nepal")
...
```{r}
res <- fit_primary_model(acs_itt_nona_dat, logistic_site_epoch, ctr = contr.treatment)
names(res$drws$AOR) <- "Nafamostat"
names(res$drws$COR) <- intervention_labels2()$CAssignment[-(1:2)]
names(res$drws$OR)[-1] <- c("Ineligible aspirin", "Age \u2265 60", "Oxygen requirement", "Australia/New Zealand", "Nepal")
```
...
names(res$drws$AOR) <- "Nafamostat"
names(res$drws$COR) <- intervention_labels2()$CAssignment[-(1:2)]
names(res$drws$OR)[-1] <- c("Age \u2265 60", "Oxygen requirement")
```
