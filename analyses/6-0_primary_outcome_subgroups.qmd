...
ggsave(
  file.path("outputs", "figures", "outcomes", "primary", "subgroup", "fas-itt-subgroup-age-hte-ppc.pdf"),
...
ggsave(
  file.path("outputs", "figures", "outcomes", "primary", "fas-itt-subgroup-oxygen-hte.pdf"),
  p, height = 3, width = 6, device = cairo_pdf)
p
```
...
```{r}
tab <- make_subgroup_summary_table(ordat, "latex")
save_tex_table(tab, "outcomes/primary/subgroup/oxygen-odds-ratios")
make_subgroup_summary_table(ordat)
```

```{r}
tab <- make_subgroup_summary_table(compdat, "latex")
save_tex_table(tab, "outcomes/primary/subgroup/oxygen-odds-ratios-compare")
make_subgroup_summary_table(compdat)
```
...
Xref <- make_X_design(
  ...
  ctr = contr.treatment)
Xref <- cbind(Xref, "randA1:rec_steroids" = Xref[, 3] * Xref[, "rec_steroids"])

X <- make_X_design(
  fas_itt_nona_dat, 
  vars = c("inelgc3", "agegte60", "sexF", "supp_oxy2", "ctry", "rec_steroids"), 
  ctr = contr.equalprior)
Xint <- cbind(X, "randA1:rec_steroids" = X[, 3] * X[, "rec_steroids"])
attr(Xint, "contrasts") <- attr(X, "contrasts")
...
N <- dim(Xint)[1]
K <- dim(Xint)[2]
beta_sd <- c(2.5, rep(1, 6), 10, 2.5, 2.5, 2.5, 1, 1, 2.5, 1)
mdat <- list(
  N = N, K = K, X = Xint, y = y_mod, y_raw = y_raw,
  J = max(y_mod), p_par = rep(2 / max(y_mod), max(y_mod)),
  M_region = M_region, region = region,
...
grp_ppc(sym("rec_steroids"))
```

```{r}
p <- plot_subgroup_or(ordat, sym("Received corticosteroids"))
ggsave(
  file.path("outputs", "figures", "outcomes", "primary", "fas-itt-subgroup-steroids-hte.pdf"),
  p, height = 3, width = 6, device = cairo_pdf)
p
```
...
Xref <- make_X_design(
  ...
  ctr = contr.treatment)
Xref <- cbind(Xref, "randA1:rec_remdesivir" = Xref[, 3] * Xref[, "rec_remdesivir"])
...
grp_ppc(sym("rec_remdesivir"))

dd <- grp_ppc(sym("rec_remdesivir")) |>
  mutate("Received remdesivir" = factor(grp, labels = c("No", "Yes")),
         "Antiviral" = factor(AAssignment, levels = c("A0", "A1", "A2"), labels = intervention_labels_short()$AAssignment))
dd %>%
    ggplot(aes(y = `Received remdesivir`, xdist = rvar_mean_y_ppc)) +
    facet_grid(Antiviral ~ .) +
    stat_interval(size = 2) +
    geom_point(aes(x = `y/n`, y = `Received remdesivir`), colour = "red", shape = 23) +
    labs(x = "Posterior predictive\nproportion")  
```
...
ggsave(
  file.path("outputs", "figures", "outcomes", "primary", "fas-itt-subgroup-remdesivir-hte.pdf"),
  p, height = 3, width = 6, device = cairo_pdf)
p
```
...
Xref <- make_X_design(
  ...
  ctr = contr.treatment)
Xref <- cbind(Xref, "randA1:rec_antiviral" = Xref[, 3] * Xref[, "rec_antiviral"])

X <- make_X_design(
  fas_itt_nona_dat, 
  vars = c("inelgc3", "agegte60", "sexF", "supp_oxy2", "ctry", "rec_antiviral"), 
  ctr = contr.equalprior)
Xint <- cbind(X, "randA1:rec_antiviral" = X[, 3] * X[, "rec_antiviral"])
attr(Xint, "contrasts") <- attr(X, "contrasts")
...
N <- dim(Xint)[1]
K <- dim(Xint)[2]
beta_sd <- c(2.5, rep(1, 6), 10, 2.5, 2.5, 2.5, 1, 1, 2.5, 10)
mdat <- list(
  N = N, K = K, X = Xint, y = y_mod, y_raw = y_raw,
  J = max(y_mod), p_par = rep(2 / max(y_mod), max(y_mod)),
  M_region = M_region, region = region,
...
dd <- grp_ppc(sym("rec_antiviral")) |>
  mutate("Received antiviral" = factor(grp, labels = c("No", "Yes")),
         "Antiviral" = factor(AAssignment, levels = c("A0", "A1", "A2"), labels = intervention_labels_short()$AAssignment))
dd %>%
    ggplot(aes(y = `Received antiviral`, xdist = rvar_mean_y_ppc)) +
    facet_grid(Antiviral ~ .) +
    stat_interval(size = 2) +
    geom_point(aes(x = `y/n`, y = `Received antiviral`), colour = "red", shape = 23) +
    labs(x = "Posterior predictive\nproportion")  
```
...
ggsave(
  file.path("outputs", "figures", "outcomes", "primary", "fas-itt-subgroup-antiviral-hte.pdf"),
  p, height = 3, width = 6, device = cairo_pdf)
p
```
...
Xref <- make_X_design(
  ...
  ctr = contr.treatment)
Xref <- cbind(Xref, "randA1:vaxfac" = Xref[, 3] * Xref[, "vaxfacYes"])

X <- make_X_design(
  fas_itt_nona_dat, 
  vars = c("inelgc3", "agegte60", "sexF", "supp_oxy2", "ctry", "vaxfac"), 
  ctr = contr.equalprior)
Xint <- cbind(X, "randA1:vaxYes" = X[, 3] * X[, "vaxfacYes"])
attr(Xint, "contrasts") <- attr(X, "contrasts")
...
N <- dim(Xint)[1]
K <- dim(Xint)[2]
beta_sd <- c(2.5, rep(1, 6), 10, 2.5, 2.5, 2.5, 1, 1, 2.5, 2.5, 1)
mdat <- list(
  N = N, K = K, X = Xint, y = y_mod, y_raw = y_raw,
  J = max(y_mod), p_par = rep(2 / max(y_mod), max(y_mod)),
  M_region = M_region, region = region,
...
seff <- rbind(
  diff(Ca) %**% (mdrws$beta["randA1"]),
  diff(Ca) %**% (mdrws$beta["randA1"] + mdrws$beta["randA1:vaxYes"]),
  diff(Ca) %**% (mdrws$beta["randA1"])
)
...
ggsave(
  file.path("outputs", "figures", "outcomes", "primary", "fas-itt-subgroup-vax-hte.pdf"),
  p, height = 3, width = 6, device = cairo_pdf)
p
```
...
```{r}
X <- model.matrix( ~ randA * agegte60, data = avs_itt_nona_dat, contrasts.arg = list(randA = contr.equalprior))
N <- dim(X)[1]
K <- dim(X)[2]
y <- avs_itt_nona_dat[["PO"]]
beta_sd <- c(2.5, 1, 2.5, 1)
mdat <- list(
  N = N, K = K, 
  X = X, y = y,
  beta_sd = beta_sd
...
mdrws$OR <- exp(seff)
rownames(mdrws$OR) <- sgroup_labs$Age
colnames(mdrws$OR) <- intervention_labels_short()$AAssignment[-(1:2)]
OR_age <- mdrws$OR
rownames(OR_age) <- sgroup_labs_shrt$Age
mdrws$compare <- sweep(mdrws$OR, 2, mdrws$OR[1, ], FUN = "/")
...
agedat <- avs_itt_dat %>% 
  ...
  mutate(
    Subgroup = "Age",
    Variable = factor(agegte60, labels = labs2),
    Intervention = as.character(factor(
      AAssignment, 
...
plot_ppc(grp_ppc(sym("agegte60"), sgroup_labs_shrt$Age), "Age") 
```

### Days since symptom onset

```{r}
X <- model.matrix( ~ randA * dsfsgt7, data = avs_itt_nona_dat, contrasts.arg = list(randA = contr.equalprior))
N <- dim(X)[1]
K <- dim(X)[2]
y <- avs_itt_nona_dat[["PO"]]
beta_sd <- c(2.5, 1, 2.5, 1)
...
mdrws$OR <- exp(seff)
rownames(mdrws$OR) <- sgroup_labs[["Days since symptom onset"]]
colnames(mdrws$OR) <- intervention_labels_short()$AAssignment[-(1:2)]
mdrws$compare <- sweep(mdrws$OR, 2, mdrws$OR[1, ], FUN = "/")
OR_dsfs <- mdrws$OR
...
ordat <- expand_grid(
  Contrast = intervention_labels_short()$AAssignment[-(1:2)],
  `Days since symptom onset` = sgroup_labs$`Days since symptom onset`,
) %>%
  mutate(Posterior = c(mdrws$OR)) %>%
  summarise_posterior()
compdat <- expand_grid(
...
dsfsdat <- avs_itt_dat %>% 
  ...
  ungroup() %>%
  mutate(
    Subgroup = "Days since symptom onset",
    Variable = factor(dsfsgt7, labels = sgroup_labs_shrt[["DSFS"]]),
    Intervention = as.character(factor(
      AAssignment, 
      labels = intervention_labels_short()$AAssignment[-1])),
    .before = 1
...
y_ppc <- mdrws$y_ppc
ppc_dat <- bind_cols(avs_itt_nona_dat, tibble(y_ppc = y_ppc))
plot_ppc(grp_ppc(sym("dsfsgt7"), sgroup_labs_shrt$DSFS), "DSFS")
```
...
```{r}
X <- model.matrix( ~ randA * supp_oxy2, data = avs_itt_nona_dat, contrasts.arg = list(randA = contr.equalprior))
N <- dim(X)[1]
K <- dim(X)[2]
y <- avs_itt_nona_dat[["PO"]]
beta_sd <- c(2.5, 1, 2.5, 1)
...
mdrws$OR <- exp(seff)
rownames(mdrws$OR) <- sgroup_labs[["Supplemental oxygen"]]
colnames(mdrws$OR) <- intervention_labels_short()$AAssignment[-(1:2)]
mdrws$compare <- sweep(mdrws$OR, 2, mdrws$OR[1, ], FUN = "/")
OR_oxygen <- mdrws$OR
...
ordat <- expand_grid(
  Contrast = intervention_labels_short()$AAssignment[-(1:2)],
  `Supplemental oxygen` = sgroup_labs$`Supplemental oxygen`,
) %>%
  mutate(Posterior = c(mdrws$OR)) %>%
  summarise_posterior()
compdat <- expand_grid(
...
oxydat <- avs_itt_dat %>% 
  ...
  mutate(
    Subgroup = "Days since symptom onset",
    Variable = factor(supp_oxy2, labels = sgroup_labs_shrt[["Oxygen"]]),
    Intervention = as.character(factor(
      AAssignment, 
      labels = intervention_labels_short()$AAssignment[-1])),
    .before = 1
...
y_ppc <- mdrws$y_ppc
ppc_dat <- bind_cols(avs_itt_nona_dat, tibble(y_ppc = y_ppc))
plot_ppc(grp_ppc(sym("supp_oxy2"), sgroup_labs_shrt$Oxygen), "Oxygen")
```
