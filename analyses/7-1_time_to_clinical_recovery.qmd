...
X <- make_X_design(fas_itt_outdat, c("inelgc3", "agegte60", "supp_oxy2", "ctry"))
Xm <- X[, -1]
...
sdat <- list(
  ...
  beta_sd = c(1, 1, 1, 1, 1, 1, 10, 2.5, 2.5, 1, 1)
)
```
...
```{r}
#| code-summary: Prepare data
X <- make_X_design(avs_itt_outdat, c("agegte60", "supp_oxy2"))
Xm <- X[, -1]
sdat <- list(
...
drws$Atrt <- drws$Acon[-1, ] - drws$Acon[1, ]
drws$Ctrt <- drws$Ccon[-1, ] - drws$Ccon[1, ]
drws$shift <- rvar_apply(sweep(drws$beta[6:7, ], 1, colMeans(sdat$X)[6:7], "*"), 2, rvar_sum)
drws$alpha_shift <- sweep(drws$alpha, 2, drws$shift, "+")
drws$OR <- exp(rbind(drws$Atrt, drws$Ctrt, drws$beta[-(1:5), ]))
rownames(drws$OR)[-(1:4)] <- c("Age \u2265 60", "Oxygen requirement")
colnames(drws$OR) <- c("Recovery", "Death")
```
