...
p2 <- fas_itt_nona_dat %>%
  ...
  labs(fill = "mMRC breathlessness scale", y = "Cumulative proportion", x = "Anticoagulation intervention")
p <- p1 | p2
...
p2 <- fas_itt_nona_dat %>%
  ...
  scale_fill_viridis_d(option = "A", direction = -1) +
  guides(fill = guide_legend(reverse = F)) +
  labs(fill = "mMRC breathlessness scale", y = "Cumulative proportion", x = "Antiviral intervention")
p <- p1 | p2
pth <- file.path("outputs", "figures", "outcomes", "secondary")
...
p2 <- ggplot(pdat, aes(agegrp, p)) +
  ...
  scale_fill_viridis_d("mMRC breathlessness scale", option = "A", direction = -1) +
  guides(fill = guide_legend(reverse = F)) +
  geom_vline(xintercept = 8.5, linetype = 2) +
  theme(axis.text.x = element_text(hjust = 1, angle = 45)) +
...
pdat <- fas_itt_nona_dat %>%
  dplyr::count(
    ...
    mrc = ordered(out_mmrc_scale, levels = 1:8), 
    .drop = F) %>%
  group_by(supp_oxy) %>%
  mutate(p = n / sum(n))
pdat2 <- pdat %>%
...
p2 <- ggplot(pdat, aes(supp_oxy, p)) +
  ...
  scale_fill_viridis_d("mMRC scale", option = "A", direction = -1) +
  guides(fill = guide_legend(reverse = F)) +
  theme(legend.key.size = unit(0.5, "lines"))
p <- p1 | p2
path <- file.path("outputs", "figures", "outcomes", "secondary")
...
#| fig-cap: |
#| Distribution of mMRC scale at day 28 by country, FAS-ITT.
#| fig-height: 4
pdat <- fas_itt_nona_dat %>%
  dplyr::count(
...
grp_ppc2 <- function(grp) {
  ...
  pivot_longer(y_1:ypp_5, names_to = c("response", "mrc"), 
...
#| code-summary: Fit primary model
res <- fit_primary_model(
  acs_itt_dat %>% mutate(out_mmrc_scale = out_mmrc_scale + 1),
  ordmod,
  outcome = "out_mmrc_scale", 
  intercept = FALSE
)
