...
```{r}
events <- bind_rows(
  saes |> select(`Patient ID`, `Date of SAE onset`, `SAE Term`, `Notes from Site`) |> mutate(Type = "SAR"),
  sars |> select(`Patient ID`, `Date of SAE onset`, `SAE Term`, `Notes from Site`) |> mutate(Type = "SAE")
) |>
...
tt |> 
  filter(any_gt55 == 1) |>
  select(StudyPatientID, Antiviral = AAssignment) |>
  left_join(
...
sars |> 
  ...
  select(`Patient ID`, `Date of SAE onset`, `Notes from Site`) |>
  mutate(`Onset Date` = as.Date(`Date of SAE onset`)) |>
  select(`Patient ID`, `Onset Date`, `Notes from Site`) |>
  kable(
    "latex",
    booktabs = TRUE, 
    align = "l"
...
nmb_dat <- all_data |>
  ...
  summarise(
    ...
    `Any non-major bleeding` = sum(D28_ABNonMajorBleeding == "Yes", na.rm = TRUE)
  ) |>
